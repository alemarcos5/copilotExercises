  ...
  async function fetchActivities() {
    try {
      ...
      const participantsHtml = details.participants.length > 0 ?
        `<ul class="participants-list">${details.participants.map(p => `<li><span class="participant-email">${p}</span><button class="remove-btn" data-email="${p}" data-activity="${name}">×</button></li>`).join("")}</ul>` :
